...
function genDESKeys(key: string) {
    ...
    for (let shift of LCS) {
        lastKey = lastKey.slice(shift, lastKey.length).concat(lastKey.slice(0, shift));
...
export const encrypt: Encrypter = ({ plaintext, key }) => {
    ...
    const keys = genDESKeys(key);

    // Pad plaintext to 64 bits multiples
    if (plaintext.length % 8 !== 0) {
        plaintext = plaintext + " ".repeat(8 - plaintext.length % 8);
    }

    const plaintextBits = stringToArrayOfBits(plaintext);
    ...
    const ciphertextArray = plaintextBlockArray.map((block) => {
        ...
        // 32-bit swap
        const block_IP_rounds_swap = block_IP_rounds.slice(32, 64).concat(block_IP_rounds.slice(0, 32));
    ...
    return ciphertextArray.map((block) => bitsToString(block)).join('');
};
...
export const decrypt: Decrypter = ({ ciphertext, key }) => {
    ...
    if (ciphertext.length % 8 !== 0) {
        ciphertext = ciphertext + " ".repeat(8 - ciphertext.length % 8);
    }

    const ciphertextBits = stringToArrayOfBits(ciphertext);
