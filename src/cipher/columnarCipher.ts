...
export const genColumnarKey: ColumnarKeyGenerator = ({ key }) => {
    ...
    key = key.split('').filter((item, pos, self) => self.indexOf(item) == pos).join('');
...
export const encrypt: Encrypter = ({ plaintext, key }) => {
    const order: number[] = genColumnarKey({key});
...
export const decrypt: Decrypter = ({ ciphertext, key }) => {
    ...
    const numCols: number = ciphertext.length / numRows;

    // Splitting the ciphertext into numRows of the size of numCols
    const rows: string[] = [];
    for (let i: number = 0; i < numRows; i++) {
        rows.push(ciphertext.slice(i * numCols, (i + 1) * numCols));
    }
