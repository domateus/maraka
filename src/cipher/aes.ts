...
const SBOX_INVERSE =[[0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb],
...
function transpondString(str:string){
    ...
    for (let i = 0; i < str.length/4/2;i++){
        const temp = str[i*2] + str[i*2+1] + str[i*2+8] + str[i*2+9] + str[i*2+16] + str[i*2+17] + str[i*2+24] + str[i*2+25];
        array.push(temp);
    }
    return array;
}
...
function rightShiftString (str:string, rightShifts:number):string{
    ...
    newStr = str.slice(str.length-rightShifts) + str.slice(0, str.length-rightShifts);
    return newStr;
}
...
function gFunction(word:string,round:number){
    if (word.length !== SIZE_OF_AES_KEY_AND_STATE/WORD_BYTELENGTH*HEXBYTES){
...
export function mix_columns(state:string,inverse:boolean) {
    ...
    for (let i = 0; i < 4; i++){
        ...
        if (!inverse){
            ...
            newState[0][i] = (gmult(0x0e,s0) ^ gmult(0x0b,s1) ^ gmult(0x0d,s2) ^ gmult(0x09,s3)).toString(16);
            newState[1][i] = (gmult(0x09,s0) ^ gmult(0x0e,s1) ^ gmult(0x0b,s2) ^ gmult(0x0d,s3)).toString(16);
            newState[2][i] = (gmult(0x0d,s0) ^ gmult(0x09,s1) ^ gmult(0x0e,s2) ^ gmult(0x0b,s3)).toString(16);
            newState[3][i] = (gmult(0x0b,s0) ^ gmult(0x0d,s1) ^ gmult(0x09,s2) ^ gmult(0x0e,s3)).toString(16);
        }
    }
